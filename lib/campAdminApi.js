...
function isMissingSeasonSettingsColumn(error) {
  return String(error?.message || '').includes('season_settings')
}
...
export async function fetchAdminConfigFromSupabase() {
  ...
  const raw = {
    ...
    seasonSettings: settingsResponse.data?.season_settings || {},
  }
...
export async function saveAdminConfigToSupabase(config) {
  ...
  const { error: settingsError } = await supabase.from('camp_admin_settings').upsert(
    {
      ...
      bootcamp_tuition_full_week: merged.tuition.bootcamp.regular.fullWeek,
      bootcamp_tuition_full_day: merged.tuition.bootcamp.regular.fullDay,
      bootcamp_tuition_am_half: merged.tuition.bootcamp.regular.amHalf,
      bootcamp_tuition_pm_half: merged.tuition.bootcamp.regular.pmHalf,
      bootcamp_discount_full_week: merged.tuition.bootcamp.discount.fullWeek,
      bootcamp_discount_full_day: merged.tuition.bootcamp.discount.fullDay,
      bootcamp_discount_am_half: merged.tuition.bootcamp.discount.amHalf,
      bootcamp_discount_pm_half: merged.tuition.bootcamp.discount.pmHalf,
  ...
  // Season overrides live in their own additive column. Saving them separately
  // keeps every other setting persistable on databases that have not run
  // sql/add_season_settings.sql yet.
  const { error: seasonError } = await supabase
    .from('camp_admin_settings')
    .upsert({ id: true, season_settings: merged.seasonSettings || {} }, { onConflict: 'id' })

  if (seasonError && !isMissingSeasonSettingsColumn(seasonError)) {
    return seasonError
  }
  ...
  if (seasonError) {
    const warning = new Error(
      'Everything saved except the per-season pricing overrides. Run sql/add_season_settings.sql in Supabase, then save again.'
    )
