...
function buildStudentPriceRows(summary, studentIndex, tuition, options = {}) {
  ...
  const siblingDiscountPct = studentIndex >= 1 ? Number(tuition?.siblingDiscountPct || 0) : 0
  ...
  const siblingAfterLimitedDiscount =
    siblingDiscountPct > 0 ? tuitionSubtotal * (siblingDiscountPct / 100) : 0
  const siblingBeforeLimitedDiscount =
    siblingDiscountPct > 0 ? subtotalRegular * (siblingDiscountPct / 100) : 0

  const totalAfterLimited = Math.max(0, tuitionSubtotal - siblingAfterLimitedDiscount) + lunchTotal
  const totalBeforeLimited =
    Math.max(0, Math.max(0, subtotalRegular - siblingBeforeLimitedDiscount) - limitedDiscountAmount) +
    lunchTotal
...
function buildStyledSummaryPdfBase64({
  ...
  function drawMarketingBox(topY) {
    ...
    drawRect(margin, topY - boxHeight, contentWidth, boxHeight, {
      fillColor: [1, 0.984, 0.922],
      strokeColor: [0.961, 0.62, 0.043],
    })
    let nextY = drawText('Encouraging Next Step', margin + 14, topY - 22, {
      font: 'F2',
      fontSize: 14,
      leading: 16,
      color: [0.604, 0.204, 0.024],
    })
    nextY = drawText(wrapped, margin + 14, nextY - 2, {
      font: 'F1',
      fontSize: 11,
      leading: 14,
...
export function buildRegistrationSummaryDocument({
  ...
  const marketingLines = [
    'Keep the same family discount when you add more camp weeks.',
    'Additional weeks help campers build stronger confidence, consistency, and visible progress.',
  ]
  ...
  const studentSections = students
    .map((student, index) => {
      ...
      const invoice = buildStudentPriceRows(summary, index, tuition, { applyLimitedDiscount })
      const {
        lunchWeeks,
        registeredDays,
        includedLunchDays,
      ...
      invoice.rows.forEach((row) => {
        pdfLines.push(
          `${row.label}: qty ${row.qty} | unit ${currency(row.effectivePrice)} | total ${currency(row.lineTotal)}`
        )
      })
      pdfLines.push(
        `Subtotal: ${currency(invoice.subtotalRegular)}`,
        `Discounts: -${currency(invoice.limitedDiscountAmount + invoice.siblingDiscountAmount)}`,
      ...
      studentPdfSections.push({
        ...
        note:
          'On paid-lunch days, our team will contact you closer to each camp week to confirm menu options. Thursday BBQ lunch is included in tuition.',
        invoiceRows: invoice.rows.map((row) => ({
          label: row.label,
          qty: row.qty,
          unitPrice: currency(row.effectivePrice),
          discount: row.discountLineTotal > 0 ? `-${currency(row.discountLineTotal)}` : '-',
          total: currency(row.lineTotal),
        })),
        subtotal: currency(invoice.subtotalRegular),
        totalDiscount: `-${currency(invoice.limitedDiscountAmount + invoice.siblingDiscountAmount)}`,
      ...
      return `
        <section class="studentSection">
          ...
          <p class="note">On paid-lunch days, our team will contact you closer to each camp week to confirm menu options. Thursday BBQ lunch is included in tuition.</p>
          <h3>Tuition Table</h3>
          <table>
          ...
          <p><strong>Total discount:</strong> -${currency(invoice.limitedDiscountAmount + invoice.siblingDiscountAmount)}</p>
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .marketingBox { border: 2px solid #f59e0b; background: #fffbeb; border-radius: 16px; padding: 14px 16px; margin-bottom: 16px; }
          .marketingBox strong { display: block; font-size: 18px; color: #9a3412; margin-bottom: 6px; }
      ...
      <body>
        <div class="marketingBox">
          <strong>Encouraging Next Step</strong>
