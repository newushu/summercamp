import { formatWeekLabel } from './campAdmin'
import {
  WEEK_TIER_PROMO,
...
import { getLunchItemLabel, isLunchItemSelected, normalizeLunchDayKey } from './lunchOptions'
import { publicWushuCopy } from './publicWushuCopy'

const INCLUDED_LUNCH_DAY_KEY = 'Fri'

/**
 * Mirrors the Train More, Save More cap the registration page applied. Callers
 * that omit `weekTierPromoQuota` (admin re-pricing of a saved registration) get
 * the uncapped behaviour.
 */
function getWeekTierPromoQuotaOptions(options = {}, student = null) {
  const quota = options.weekTierPromoQuota
  if (!quota) {
    return {}
  }
  return {
    quotaRemaining: quota.remaining,
    quotaSlotIndex: quota.claimOrder?.[student?.id] || 0,
  }
}
...
function getCanonicalWeekId(programKey, weekId) {
  ...
  if (rawWeekId.startsWith('daycamp:')) {
    ...
    return `${programKey || 'general'}:${startDate}`
  }
  if (/^\d{4}-\d{2}-\d{2}$/.test(rawWeekId)) {
    return `${programKey || 'general'}:${rawWeekId}`
  }
  return rawWeekId
}
...
function buildWeekDaysFromStart(startDate) {
  const start = parseDateLocal(startDate)
  if (!start) {
    return ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((dayKey) => ({ key: dayKey, date: '' }))
  }
  return ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((dayKey, index) => ({
    key: dayKey,
...
function getStudentSummary(student) {
  ...
  const lunchCount = Object.entries(student?.lunch || {}).filter(
    ([key, value]) => isLunchItemSelected(value) && isPaidLunchSelectionKey(key)
  ).length
...
function getLunchWeeksForStudent(student, weeksById) {
  ...
  for (const [weekId, entry] of Object.entries(student?.schedule || {})) {
    const entryProgramKey = entry?.campType === 'bootcamp' || entry?.programKey === 'bootcamp' ? 'bootcamp' : 'general'
    const canonicalWeekId = getCanonicalWeekId(entryProgramKey, weekId)
    const week = weeksById?.[canonicalWeekId] || weeksById?.[weekId] || weeksById?.[getDatePartFromWeekId(weekId)]
    ...
    const sourceDays =
      Array.isArray(week?.days) && week.days.length > 0
        ? week.days
        : buildWeekDaysFromStart(week?.start || getDatePartFromWeekId(canonicalWeekId || weekId))
    const dayMetaByKey = sourceDays.reduce((acc, day) => {
      acc[String(day?.key || '').trim()] = day
      return acc
    }, {})
    const selectedDays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']
      .map((dayKey) => ({ dayKey, mode: getDayMode(entry, dayKey) }))
      .filter((day) => day.mode && day.mode !== 'NONE')
      .map(({ dayKey, mode }) => {
        const normalizedDayKey = normalizeLunchDayKey(dayKey)
        const dayMeta = dayMetaByKey[normalizedDayKey] || dayMetaByKey[dayKey] || {}
        return {
    ...
    if (selectedDays.length > 0) {
      rows.push({
        ...
        week:
          week || {
            id: canonicalWeekId || weekId,
            start: getDatePartFromWeekId(canonicalWeekId || weekId),
            end: isoDate(addDays(parseDateLocal(getDatePartFromWeekId(canonicalWeekId || weekId)) || new Date(), 4)),
  ...
  const paidLunchDays = lunchWeeks.reduce(
    (sum, row) =>
      ...
      row.selectedDays.filter(
        (day) => !isIncludedLunchDay(day.dayKey) && isLunchItemSelected(student?.lunch?.[day.key] || student?.lunch?.[day.canonicalKey])
      ).length,
    0
  )
...
function buildStudentPriceRows(summary, studentIndex, tuition, options = {}) {
  ...
  const rows = [
    ['general-fullWeek', 'fullWeek', 'General Summer Wushu Week Full Week', summary.general.fullWeeks, 'general'],
    ['general-fullDay', 'fullDay', 'General Summer Wushu Week Full Day', summary.general.fullDays, 'general'],
    ['general-amHalf', 'amHalf', 'General Summer Wushu Week AM Half Day', summary.general.amDays, 'general'],
    ['general-pmHalf', 'pmHalf', 'General Summer Wushu Week PM Half Day', summary.general.pmDays, 'general'],
    ...
    .map(([id, key, label, qty, rateType]) => {
      const configuredRate = getCampRateForLocation(options.location, rateType, key, tuition, {
        discountCampaignId,
      })
  ...
  const fourthWeekPromo = student
    ? getWeekTierPromoForStudent(student, options.weeksById, tuition, {
        ...
        discountCampaignId,
        ...getWeekTierPromoQuotaOptions(options, student),
      })
...
function buildStyledSummaryPdfBase64({
  ...
  function drawAccountingSummary(summary) {
    if (!summary) {
      return
    }
    ...
    ensureSpace(46 + rows.length * 18)
    drawSectionTitle(summary.title || 'Current Accounting Snapshot')
    drawTable(
      ['Item', 'Amount'],
  ...
  startPage(true)
  drawAccountingSummary(accountingSummary)
  ...
  for (const section of studentSections || []) {
    ...
    if (section.lunchLines.length === 0) {
      drawWrappedParagraph('No summer wushu week days selected yet.')
    } else {
  ...
  ensureSpace(120)
  drawSectionTitle('Family Summer Wushu Week & Lunch Calendar')
  drawWrappedParagraph('Each summer wushu week day shows lunch plan and notable reminders.', {
    fontSize: 10,
    color: [0.286, 0.333, 0.412],
  })
  cursorY -= 4
...
export function buildRegistrationSummaryDocument({
  ...
  const marketingLines = [
    ...getWeekTierPromoLines(),
    'Discounts are calculated automatically per student and arranged to preserve the highest qualifying total.',
  ]
  ...
  const familyMetaRowsHtml = [
    ...
    .join('')
  const accountingSummaryRows = Array.isArray(accountingSummary?.rows) ? accountingSummary.rows : []
  const accountingSummaryHtml =
    accountingSummaryRows.length > 0
      ? `
        <section class="studentSection accountingSummarySection">
          <div class="studentHeaderRow">
            <h2>${escapeHtml(accountingSummary?.title || 'Current Accounting Snapshot')}</h2>
          </div>
          <table class="summaryTable accountingSummaryTable">
            <tbody>
              ${accountingSummaryRows
                .map(
                  (row) => `
                    <tr>
  ...
  const studentSections = students
    .map((student, index) => {
      ...
      pdfLines.push(
        ...
        `Fri included lunch: ${includedLunchDays} day(s)`,
      ...
      const lunchCalendarHtml = lunchWeeks
        .map((row, weekIndex) => {
          const dayLines = row.selectedDays
            .map((day) => {
              const selectedLunch = student.lunch?.[day.key] || student.lunch?.[day.canonicalKey]
              const hasLunch = isLunchItemSelected(selectedLunch)
              ...
              const lunchText = isIncludedLunch
                ...
                : hasLunch
                  ? getLunchItemLabel(selectedLunch)
                  : 'Pack lunch needed'
              pdfLines.push(
          ...
          return `
            <section class="weekBlock">
              ...
              <table>
                <thead><tr><th>Day</th><th>Date</th><th>Wushu Week Schedule</th><th>Lunch Plan</th><th>Notable</th></tr></thead>
      ...
      studentPdfSections.push({
        ...
        note:
          `Summer Wushu Week notes: On paid-lunch days, our team will contact you closer to each week to confirm menu options. Tuesday outdoor time may need sunscreen and a change of outdoor shoes. Friday BBQ lunch is included in tuition, and Friday showcase is the same day.`,
      ...
      return `
        <section class="studentSection">
          ...
          <div class="metricGrid">
            ...
            <div class="metricCell"><span>Fri included lunch</span><strong>${includedLunchDays} day(s)</strong></div>
          ...
          ${invoice.nextWeekTierPromoPrompt?.eligible
            ...
            : ''}
          <div class="noteBox">
            <strong>Summer Wushu Week Notes</strong>
            <p>On paid-lunch days, our team will contact you closer to each week to confirm menu options. Tuesday outdoor time may need sunscreen and a change of outdoor shoes. Friday BBQ lunch is included in tuition, and Friday showcase is the same day.</p>
          </div>
        ...
        for (let index = 0; index < 7; index += 1) {
          ...
          weekCells.push(`
            <div class="calendarCell ${inRange ? '' : 'muted'}">
              ...
              ${itemsHtml ? `<ul>${itemsHtml}</ul>` : '<p class="emptyCell">No wushu program</p>'}
            </div>
          `)
  ...
  pdfLines.push('', 'Family Summer Wushu Week & Lunch Calendar')
  allCampEntries
    ...
    <html>
      ...
      <body>
        <div class="heroHeader">
          ...
          <h1>Summer Wushu Week Registration Summary</h1>
        ...
        <p class="note">Each summer wushu week day shows lunch plan and notable reminders.</p>
        ${familyCalendarHtml}
      </body>
    </html>
  `
  ...
  return {
    ...
    pdfBase64: buildStyledSummaryPdfBase64({
      title: 'Summer Wushu Week Registration Summary',
      marketingLines,
      businessLines,
      ...
      accountingSummary:
        accountingSummaryRows.length > 0
          ? {
              title: accountingSummary?.title || 'Current Accounting Snapshot',
    ...
    plainText: [
      ...
      ...(accountingSummaryRows.length > 0
        ? [
            accountingSummary?.title || 'Current Accounting Snapshot',
            ...accountingSummaryRows.map((row) => `${row.label}: ${row.value}`),
            '',
          ]
        : []),
      ...pdfLines,
...
export function buildRegistrationSummarySnapshot({
  ...
  const amountDue = students.reduce((sum, student, index) => {
    ...
    summaryLines.push(
      publicWushuCopy(`${camperName}: ${invoice.rows.length > 0 ? invoice.rows.map((row) => `${row.label} x${row.qty}`).join(', ') : 'No camp days selected'}`)
    )
