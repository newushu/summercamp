...
function normalizeCampaignDiscountEndDate(value) {
  ...
  if (!raw) {
    return ROUND_TWO_DISCOUNT_END_DATE
  }
  return raw < ROUND_TWO_DISCOUNT_END_DATE ? ROUND_TWO_DISCOUNT_END_DATE : raw
}
...
export function getLimitedDiscountCampaigns(discountEndDate = ROUND_TWO_DISCOUNT_END_DATE) {
  const roundTwoEndDate = normalizeCampaignDiscountEndDate(discountEndDate)
  return [
    {
...
export function getDiscountCampaignMeta(discountCampaignId, discountEndDate = ROUND_TWO_DISCOUNT_END_DATE) {
...
export function resolveDiscountCampaignId({
  createdAt,
  discountEndDate = ROUND_TWO_DISCOUNT_END_DATE,
  overrideId = '',
} = {}) {
...
function normalizeRoundTwoFullWeekPrice(regularValue, fallbackValue = 0) {
  const regular = Number(regularValue || 0)
  if (regular <= 0) {
    return Number(fallbackValue || 0)
  }
  return roundTuitionToFive(Math.max(0, regular - ROUND_TWO_FULL_WEEK_DISCOUNT_AMOUNT))
}
...
export function mergeAdminConfig(raw = {}) {
  ...
  const discountFullWeek = normalizeRoundTwoFullWeekPrice(regularFullWeek, raw.tuition?.discount?.fullWeek)
  ...
  const bootcampDiscountFullWeek = normalizeRoundTwoFullWeekPrice(
