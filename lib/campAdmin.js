export const defaultAdminConfig = {
  ...
  // Per-season overrides keyed by year, e.g. { '2027': { tuition, heroBanner... } }.
  // Anything left blank or at 0 inherits the base configuration above.
  seasonSettings: {},
}

export const ROUND_TWO_FULL_WEEK_DISCOUNT_AMOUNT = 75
export const ROUND_TWO_DISCOUNT_END_DATE = '2026-06-30'
export const ROUND_TWO_DISCOUNT_NAME = 'ROUND 2 Early Bird Special'
export const ROUND_THREE_FULL_WEEK_DISCOUNT_AMOUNT = 50
export const ROUND_THREE_DISCOUNT_END_DATE = '2026-08-25'
export const ROUND_THREE_DISCOUNT_NAME = 'ROUND 3 Summer Special'
export const ROUND_ONE_FULL_WEEK_DISCOUNT_AMOUNT = 100
export const ROUND_ONE_DISCOUNT_END_DATE = '2026-05-20'
export const ROUND_ONE_DISCOUNT_NAME = 'ROUND 1 Early Bird Special'

export const LIMITED_DISCOUNT_CAMPAIGN_IDS = {
  ROUND_ONE: 'round1',
  ROUND_TWO: 'round2',
  ROUND_THREE: 'round3',
}
...
function normalizeCampaignDiscountEndDate(value) {
  const raw = typeof value === 'string' ? value.trim() : ''
  if (!raw) {
    return ROUND_THREE_DISCOUNT_END_DATE
  }
  return raw < ROUND_THREE_DISCOUNT_END_DATE ? ROUND_THREE_DISCOUNT_END_DATE : raw
}
...
export function getLimitedDiscountCampaigns(discountEndDate = ROUND_THREE_DISCOUNT_END_DATE) {
  const roundThreeEndDate = normalizeCampaignDiscountEndDate(discountEndDate)
  return [
    {
      id: LIMITED_DISCOUNT_CAMPAIGN_IDS.ROUND_ONE,
      name: ROUND_ONE_DISCOUNT_NAME,
      fullWeekDiscountAmount: ROUND_ONE_FULL_WEEK_DISCOUNT_AMOUNT,
      startsAt: '',
      endsAt: ROUND_ONE_DISCOUNT_END_DATE,
    },
    {
      id: LIMITED_DISCOUNT_CAMPAIGN_IDS.ROUND_TWO,
      name: ROUND_TWO_DISCOUNT_NAME,
      fullWeekDiscountAmount: ROUND_TWO_FULL_WEEK_DISCOUNT_AMOUNT,
      startsAt: '2026-05-21',
      endsAt: ROUND_TWO_DISCOUNT_END_DATE,
    },
    {
      id: LIMITED_DISCOUNT_CAMPAIGN_IDS.ROUND_THREE,
      name: ROUND_THREE_DISCOUNT_NAME,
      fullWeekDiscountAmount: ROUND_THREE_FULL_WEEK_DISCOUNT_AMOUNT,
      startsAt: '2026-07-01',
      endsAt: roundThreeEndDate,
    },
  ]
}

export function normalizeDiscountCampaignOverride(value) {
  const normalized = String(value || '').trim().toLowerCase()
  return getLimitedDiscountCampaigns()
    .map((item) => item.id)
    .includes(normalized)
    ? normalized
    : ''
...
export function getDiscountCampaignMeta(discountCampaignId, discountEndDate = ROUND_THREE_DISCOUNT_END_DATE) {
  const normalizedId = normalizeDiscountCampaignOverride(discountCampaignId)
  if (!normalizedId) {
    return null
  }
  return getLimitedDiscountCampaigns(discountEndDate).find((item) => item.id === normalizedId) || null
}

export function resolveDiscountCampaignId({
  createdAt,
  discountEndDate = ROUND_THREE_DISCOUNT_END_DATE,
  overrideId = '',
} = {}) {
  const explicitOverride = getDiscountCampaignMeta(overrideId, discountEndDate)
  if (explicitOverride) {
    return explicitOverride.id
  }

  const createdDate = normalizeDateOnly(createdAt)
  if (!createdDate) {
    return ''
  }

  return (
    getLimitedDiscountCampaigns(discountEndDate).find((campaign) => {
      const startsAt = normalizeDateOnly(campaign.startsAt)
      const endsAt = normalizeDateOnly(campaign.endsAt)
      if (startsAt && createdDate < startsAt) {
        return false
      }
      if (endsAt && createdDate > endsAt) {
        return false
      }
      return true
    })?.id || ''
...
export function isLimitedDiscountActiveForDate(
  discountEndDate = ROUND_THREE_DISCOUNT_END_DATE,
  createdAt,
  overrideId = ''
) {
  return Boolean(resolveDiscountCampaignId({ createdAt, discountEndDate, overrideId }))
}

function normalizeCurrentFullWeekDiscountPrice(regularValue, fallbackValue = 0) {
  const regular = Number(regularValue || 0)
  if (regular <= 0) {
    return Number(fallbackValue || 0)
  }
  return roundTuitionToFive(Math.max(0, regular - ROUND_THREE_FULL_WEEK_DISCOUNT_AMOUNT))
}
...
export function getTuitionForDiscountCampaign(tuition = {}, discountCampaignId = '') {
  const campaign = getDiscountCampaignMeta(discountCampaignId, tuition?.discountEndDate)
  if (!campaign) {
    return tuition
  }

  const amount = Number(campaign.fullWeekDiscountAmount || 0)
  const regularFullWeek = Number(tuition?.regular?.fullWeek || 0)
  const regularOvernightWeek = Number(tuition?.regular?.overnightWeek || 0)
  const resolvedBootcamp = resolveBootcampTuition(tuition)
  const bootcampRegularFullWeek = Number(resolvedBootcamp.regular?.fullWeek || 0)
  ...
  return {
    ...
    discount: {
      ...
      fullWeek:
        regularFullWeek > 0
          ? roundTuitionToFive(Math.max(0, regularFullWeek - amount))
          : Number(tuition?.discount?.fullWeek || 0),
      overnightWeek:
        regularOvernightWeek > 0
          ? roundTuitionToFive(Math.max(0, regularOvernightWeek - amount))
          : Number(tuition?.discount?.overnightWeek || 0),
    },
    bootcamp: {
      ...(tuition?.bootcamp || {}),
      regular: {
        ...(tuition?.bootcamp?.regular || {}),
      },
      discount: {
        ...(tuition?.bootcamp?.discount || {}),
        fullWeek:
          bootcampRegularFullWeek > 0
            ? roundTuitionToFive(Math.max(0, bootcampRegularFullWeek - amount))
            : Number(tuition?.bootcamp?.discount?.fullWeek || 0),
      },
    },
  }
}
...
const seasonTuitionKeys = ['fullWeek', 'fullDay', 'amHalf', 'pmHalf', 'overnightWeek', 'overnightDay']
const seasonBootcampKeys = ['fullWeek', 'fullDay', 'amHalf', 'pmHalf']

export function createEmptySeasonSettings() {
  const zeroed = (keys) => keys.reduce((acc, key) => ({ ...acc, [key]: 0 }), {})
  return {
    tuition: {
      regular: zeroed(seasonTuitionKeys),
      bootcamp: { regular: zeroed(seasonBootcampKeys) },
      lunchPrice: 0,
    },
    heroBannerImageUrl: '',
    heroBannerVideoUrl: '',
...
function normalizeSeasonEntry(raw = {}) {
  const base = createEmptySeasonSettings()
  const readAmounts = (source, keys) =>
    keys.reduce((acc, key) => {
      const value = toNumber(source?.[key])
      return { ...acc, [key]: value > 0 ? value : 0 }
    }, {})

  return {
    tuition: {
      regular: readAmounts(raw?.tuition?.regular, seasonTuitionKeys),
      bootcamp: { regular: readAmounts(raw?.tuition?.bootcamp?.regular, seasonBootcampKeys) },
      lunchPrice: Math.max(0, toNumber(raw?.tuition?.lunchPrice)),
    },
    heroBannerImageUrl: typeof raw?.heroBannerImageUrl === 'string' ? raw.heroBannerImageUrl : base.heroBannerImageUrl,
    heroBannerVideoUrl: typeof raw?.heroBannerVideoUrl === 'string' ? raw.heroBannerVideoUrl : base.heroBannerVideoUrl,
    heroBannerHeadline: typeof raw?.heroBannerHeadline === 'string' ? raw.heroBannerHeadline : base.heroBannerHeadline,
    heroBannerSubhead: typeof raw?.heroBannerSubhead === 'string' ? raw.heroBannerSubhead : base.heroBannerSubhead,
  }
}
...
export function normalizeSeasonSettings(raw) {
  ...
  return Object.entries(raw).reduce((acc, [key, value]) => {
    ...
    acc[year] = normalizeSeasonEntry(value)
    return acc
  }, {})
}

export function mergeAdminConfig(raw = {}) {
  const bootcampPremiumPct = Math.max(0, toNumber(raw.tuition?.bootcampPremiumPct, 25))
  const regularFullWeek = toNumber(raw.tuition?.regular?.fullWeek)
  const discountFullWeek = normalizeCurrentFullWeekDiscountPrice(regularFullWeek, raw.tuition?.discount?.fullWeek)
  const regularOvernightWeek = toNumber(raw.tuition?.regular?.overnightWeek)
  const discountOvernightWeek = normalizeCurrentFullWeekDiscountPrice(
    regularOvernightWeek,
    raw.tuition?.discount?.overnightWeek
  )
  const explicitBootcampRegularFullWeek = toNumber(raw.tuition?.bootcamp?.regular?.fullWeek)
  const bootcampRegularFullWeek =
    explicitBootcampRegularFullWeek > 0
      ? explicitBootcampRegularFullWeek
      : roundTuitionToFive(regularFullWeek * (1 + bootcampPremiumPct / 100))
  const bootcampDiscountFullWeek = normalizeCurrentFullWeekDiscountPrice(
    bootcampRegularFullWeek,
    raw.tuition?.bootcamp?.discount?.fullWeek
  )
  ...
  const emailJourney = Array.from({ length: 5 }).map((_, index) => {
    ...
    return {
      ...
      imageUrl: typeof incoming.imageUrl === 'string' ? incoming.imageUrl : base.imageUrl,
  ...
  return {
    ...
    tuition: {
      regular: {
        ...
        overnightWeek: regularOvernightWeek,
        overnightDay: toNumber(raw.tuition?.regular?.overnightDay),
      },
      discount: {
        fullWeek: discountFullWeek,
        ...
        overnightWeek: discountOvernightWeek,
      ...
      bootcamp: resolveBootcampTuition({
        ...
        bootcamp: {
          regular: {
            fullWeek: explicitBootcampRegularFullWeek,
          ...
          discount: {
            fullWeek: bootcampDiscountFullWeek,
      ...
      discountEndDate:
        normalizeCampaignDiscountEndDate(raw.tuition?.discountEndDate),
    ...
    seasonSettings: normalizeSeasonSettings(raw.seasonSettings),
  }
}
