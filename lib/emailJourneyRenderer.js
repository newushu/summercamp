      ...
      <div style="max-width:700px;margin:0 auto;background:#ffffff;border:1px solid #fde68a;border-radius:22px;overflow:hidden;box-shadow:0 24px 60px rgba(194,65,12,0.14);font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;">
        ...
        <div style="padding:26px 24px 20px;">
          ...
          ${renderCard({
            title: 'Level Up App',
            tone: 'blue',
            bodyHtml: '<p style="margin:0;">Families get access to Level Up on June 20 for daily photos and videos, instructor notes, progress updates, and lunch booking — all in one place.</p>',
          })}
          ${buildCampaignCtaHtml(ctaLink, {
...
function buildReservationStepContent({ firstName, stepNumber, reservationDeadlineLabel, summaryLines, amountDue, payload }) {
  ...
  const bodyText = [...bodyLines, '', 'Family rewards:', ...getUpsellOfferLines(payload), '', ...getLevelUpRewardsLines(payload), '', 'Level Up app:', 'Families get access on June 20 for lunch booking, progress photos/videos, and instructor notes.', '', 'Payment Methods:', PAYMENT_METHODS_TEXT, '', 'Registration Summary:', ...(summaryLines || [])].join('\n')
