import { WEEK_TIER_PROMO, getWeekTierPromoLines } from './campPricing'
import {
  getRoundTwoOfferCopy,
  getRoundTwoTrainMoreSaveMoreCopy,
  getRoundTwoWushuBenefitsCopy,
} from './roundTwoCampaign'
import { publicWushuCopy } from './publicWushuCopy'

const CAMP_NAME = 'Summer Wushu Week'
...
function isRegistrationPromptLine(line) {
...
function buildRoundTwoCountdownHtml(tokens = {}) {
  const countdownLabel = String(tokens?.round_two_countdown_label || '').trim()
  const deadlineLabel = String(tokens?.round_two_deadline || '').trim()
  if (!countdownLabel) {
    return ''
  }
  return `
    <div style="margin:16px 0 0;padding:18px 20px;border:2px solid #f97316;border-radius:18px;background:linear-gradient(135deg,#fff7ed 0%,#ffedd5 100%);box-shadow:0 12px 28px rgba(249,115,22,0.12);text-align:center;">
      <p style="margin:0 0 8px;font-size:12px;font-weight:900;letter-spacing:0.14em;text-transform:uppercase;color:#9a3412;">Round 2 Deadline</p>
      <p style="margin:0;font-size:32px;line-height:1.1;font-weight:900;color:#7c2d12;">${escapeHtml(countdownLabel)}</p>
      <p style="margin:8px 0 0;font-size:14px;font-weight:700;color:#9a3412;">Offer ends ${escapeHtml(deadlineLabel)}</p>
      <p style="margin:6px 0 0;font-size:13px;color:#c2410c;">Register before the deadline to keep the current weekly discount.</p>
    </div>
  `
}

function buildCampaignCtaHtml(ctaLink, {
  title = 'Claim Your $75 Off Per Week',
  buttonLabel = 'Claim Your $75 Off Per Week',
  tone = 'blue',
  compact = false,
} = {}) {
  const isBlue = tone === 'blue'
  const wrapperStyles = isBlue
    ? 'border:2px solid #0284c7;background:linear-gradient(135deg,#0284c7 0%,#2563eb 100%);box-shadow:0 12px 32px rgba(2,132,199,0.28);'
    : 'border:2px solid #fdba74;background:linear-gradient(180deg,#fff7ed 0%,#ffedd5 100%);box-shadow:0 10px 24px rgba(249,115,22,0.12);'
  const titleColor = isBlue ? '#ffffff' : '#7c2d12'
  const subColor = isBlue ? 'rgba(255,255,255,0.88)' : '#9a3412'
  const buttonBg = isBlue ? '#ffffff' : '#ea580c'
  const buttonColor = isBlue ? '#1d4ed8' : '#ffffff'
  return `
    <div style="margin:${compact ? '16px' : '20px'} 0 0;padding:${compact ? '18px 18px' : '20px 22px'};border-radius:18px;${wrapperStyles}text-align:center;">
      <p style="margin:0 0 10px;font-size:${compact ? '18px' : '20px'};font-weight:900;color:${titleColor};">${escapeHtml(title)}</p>
      <a href="${escapeHtml(ctaLink)}" style="display:inline-block;padding:14px 28px;border-radius:999px;background:${buttonBg};color:${buttonColor};text-decoration:none;font-weight:900;font-size:16px;letter-spacing:0.01em;box-shadow:0 4px 16px rgba(15,23,42,0.14);">
        ${escapeHtml(buttonLabel)} &#8594;
      </a>
      <p style="margin:10px 0 0;font-size:12px;color:${subColor};font-weight:700;">summer.newushu.com/register</p>
    </div>
  `
}
...
function renderFeatureGrid(items, tone = 'neutral') {
  const colors = getToneColors(tone)
  return `
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">
      ${items
        .map((item) => {
          ...
          const [labelPart, detailPart = ''] = raw.split(/:\s*/, 2)
          const label = labelPart.replace(/^- /, '').trim()
          const detail = detailPart.trim()
          return `
            <div style="padding:14px 14px 12px;border:1px solid ${colors.border};border-radius:16px;background:rgba(255,255,255,0.88);box-shadow:0 6px 18px rgba(15,23,42,0.04);">
              <p style="margin:0 0 6px;font-size:12px;font-weight:900;letter-spacing:0.04em;text-transform:uppercase;color:${colors.accent};">${escapeHtml(label)}</p>
              <p style="margin:0;font-size:14px;line-height:1.55;color:${colors.text};">${escapeHtml(detail || label)}</p>
            </div>
          `
        })
        .join('')}
...
function renderSavingsDiagram() {
  const tiers = [
    { label: 'Weeks 1-3', value: 'Standard Early Bird', note: '$75 off per full week' },
    { label: 'Weeks 4-6', value: '50% OFF', note: 'Eligible full-week enrollments' },
    { label: 'Weeks 7-9', value: '60% OFF', note: 'Build stronger summer momentum' },
    { label: 'Week 10', value: 'FREE', note: 'Best long-plan value' },
  ]
  return `
    <div style="margin:14px 0 0;padding:16px;border:1px solid #bfdbfe;border-radius:18px;background:linear-gradient(180deg,#f8fbff 0%,#eef6ff 100%);">
      <p style="margin:0 0 12px;font-size:13px;font-weight:900;letter-spacing:0.05em;text-transform:uppercase;color:#1d4ed8;">Savings Path</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;">
        ${tiers
          .map(
            (tier) => `
              <div style="padding:14px 12px;border-radius:16px;border:1px solid #bfdbfe;background:#ffffff;text-align:center;box-shadow:0 6px 18px rgba(29,78,216,0.06);">
                <p style="margin:0;font-size:11px;font-weight:900;letter-spacing:0.04em;text-transform:uppercase;color:#1d4ed8;">${escapeHtml(tier.label)}</p>
                <p style="margin:8px 0 6px;font-size:24px;font-weight:900;color:#0f172a;">${escapeHtml(tier.value)}</p>
                <p style="margin:0;font-size:12px;line-height:1.5;color:#475569;">${escapeHtml(tier.note)}</p>
              </div>
            `
          )
          .join('')}
...
function buildRegistrationStepsHtml(ctaLink) {
  const steps = [
    { step: '1', title: 'Choose weeks', note: 'Pick the best schedule for your family.' },
    { step: '2', title: 'Complete registration', note: 'Use the online form at summer.newushu.com/register.' },
    { step: '3', title: 'Lock in pricing', note: 'Secure early-bird and multi-week savings when eligible.' },
  ]
  return `
    <div style="margin:18px 0 0;padding:18px 18px 16px;border:1px solid #fde68a;border-radius:20px;background:linear-gradient(180deg,#fffdf4 0%,#fff8df 100%);box-shadow:0 10px 24px rgba(234,88,12,0.05);">
      <p style="margin:0 0 12px;font-size:13px;font-weight:900;letter-spacing:0.05em;text-transform:uppercase;color:#b45309;">How To Register</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;">
        ${steps
          .map(
            (item) => `
              <div style="padding:14px 12px;border-radius:16px;border:1px solid #fcd34d;background:#ffffff;">
                <div style="width:30px;height:30px;border-radius:999px;background:#ea580c;color:#ffffff;font-size:14px;font-weight:900;line-height:30px;text-align:center;">${escapeHtml(item.step)}</div>
                <p style="margin:10px 0 4px;font-size:15px;font-weight:800;color:#0f172a;">${escapeHtml(item.title)}</p>
                <p style="margin:0;font-size:13px;line-height:1.5;color:#475569;">${escapeHtml(item.note)}</p>
              </div>
            `
          )
          .join('')}
      </div>
      <p style="margin:14px 0 0;font-size:13px;font-weight:700;color:#7c2d12;">Registration link: <a href="${escapeHtml(ctaLink)}" style="color:#b45309;text-decoration:underline;">${escapeHtml(ctaLink)}</a></p>
      ${buildCampaignCtaHtml(ctaLink, {
...
function renderStructuredNarrativeGroup(group, index) {
  ...
  const normalizedTitle = String(titleLine || '').trim().toLowerCase()

  if (normalizedTitle.startsWith('why start wushu now') && bulletItems.length > 0) {
    return renderCard({
      title: 'Why Families Start Wushu',
      tone: 'green',
      bodyHtml: `
        ${paragraphLines.slice(1).map((line) => `<p style="margin:0 0 12px;">${escapeHtml(line)}</p>`).join('')}
        ${renderFeatureGrid(bulletItems, 'green')}
      `,
    })
  }

  if (normalizedTitle.startsWith('train more, save more') && bulletItems.length > 0) {
    return renderCard({
      title: 'Train More, Save More',
      tone: 'blue',
      bodyHtml: `
        ${paragraphLines.slice(1).map((line) => `<p style="margin:0 0 12px;">${escapeHtml(line)}</p>`).join('')}
        ${renderSavingsDiagram()}
        <div style="margin:14px 0 0;">${renderFeatureGrid(bulletItems, 'blue')}</div>
      `,
    })
  }
  ...
  if (bulletItems.length > 0) {
    return renderCard({
      ...
      tone: /offer|save|week|discount/i.test(paragraphLines[0] || '') ? 'blue' : 'green',
      bodyHtml: `
        ${paragraphLines.slice(1).map((line) => `<p style="margin:0 0 10px;">${escapeHtml(line)}</p>`).join('')}
        ${renderFeatureGrid(bulletItems, /offer|save|week|discount/i.test(paragraphLines[0] || '') ? 'blue' : 'green')}
      `,
    })
  }
  ...
  const sectionHtml = sections
    .map((group, index) => {
      const cleaned = group
        ...
        .filter((line) => line && !looksLikeUrl(line) && !isRegistrationPromptLine(line))
      if (cleaned.length === 0) return ''
      ...
      if (cleaned.length === 0) return ''
      return renderStructuredNarrativeGroup(cleaned, index)
    })
    .filter(Boolean)
    .join('')
...
export function buildLeadJourneyEmailHtml({
  ...
  const offerHtml = buildFourthWeekOfferHtml('Train More, Save More')
  const footerHtml = buildEmailFooterHtml({ showReply: true })
  const registrationStepsHtml = buildRegistrationStepsHtml(ctaLink)
  const topCtaHtml = buildCampaignCtaHtml(ctaLink, {
    title: 'Claim Your $75 Off Per Week',
    buttonLabel: 'Claim Your $75 Off Per Week',
    tone: 'blue',
  })
  const midCtaHtml = buildCampaignCtaHtml(ctaLink, {
    title: 'Reserve Your Weeks Before The Deadline',
      ...
      <div style="max-width:700px;margin:0 auto;background:#ffffff;border:1px solid #fde68a;border-radius:22px;overflow:hidden;box-shadow:0 24px 60px rgba(194,65,12,0.14);font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;">
        ...
        <div style="padding:26px 24px 20px;">
          ...
          ${renderCard({
            title: 'Level Up App',
            tone: 'blue',
            bodyHtml: '<p style="margin:0;">Families get access to Level Up throughout the summer for daily photos and videos, instructor notes, progress updates, and lunch booking — all in one place.</p>',
          })}
          ${buildCampaignCtaHtml(ctaLink, {
            title: 'Ready To Choose Your Weeks?',
            buttonLabel: 'Go To Registration',
            tone: 'warm',
          })}
          ${footerHtml}
...
export function buildLeadJourneyMessage({
  ...
  const baseBody = String(template?.body || '')
  const normalizedTokens = {
    ...tokens,
    round_two_offer: tokens?.round_two_offer || getRoundTwoOfferCopy(),
    train_more_save_more: tokens?.train_more_save_more || getRoundTwoTrainMoreSaveMoreCopy(),
    wushu_why_start: tokens?.wushu_why_start || getRoundTwoWushuBenefitsCopy(),
  }
  const withRoundTwoOffer = baseBody.includes('{round_two_offer}') ? baseBody : `${baseBody}\n\n{round_two_offer}`
  const withWushuBenefits = withRoundTwoOffer.includes('{wushu_why_start}')
    ? withRoundTwoOffer
    : `${withRoundTwoOffer}\n\n{wushu_why_start}`
  const completeBody = withWushuBenefits.includes('{train_more_save_more}')
    ? withWushuBenefits
    : `${withWushuBenefits}\n\n{train_more_save_more}`
  const bodyText = renderTemplate(completeBody, normalizedTokens)
  const ctaLink = normalizedTokens?.registration_link || 'https://summer.newushu.com/register'
  const recommendation = normalizedTokens?.recommended_plan || ''
  const countdownHtml = buildRoundTwoCountdownHtml(normalizedTokens)
  const html = buildLeadJourneyEmailHtml({
    ...
    recommendation,
    logoUrl,
    heroImageUrl: String(heroImageUrl || '').trim() || pickJourneyImage(landingCarouselImageUrls, stepNumber - 1),
    videoUrl: template?.videoUrl || '',
    countdownHtml,
  })
  return { subject: publicWushuCopy(subject), bodyText: publicWushuCopy(bodyText), html: publicWushuCopy(html) }
}
...
function buildFourthWeekOfferHtml(title = 'Special Offer') {
  return buildInfoCard({
    title,
    accentColor: '#9a3412',
    background: 'linear-gradient(180deg,#fff7ed 0%,#ffedd5 100%)',
    borderColor: '#fdba74',
    bodyHtml: getWeekTierPromoLines()
      .map((line) => `<p style="margin:0 0 8px;color:#7c2d12;">${escapeHtml(line)}</p>`)
      .join(''),
  })
}
...
function buildRegistrationSummaryHtml(summaryLines) {
  ...
  for (const rawLine of normalizedLines) {
    ...
    if (
      ...
      line.startsWith('Offer reminder:') ||
      line.startsWith('Offer details:') ||
      line.startsWith('Why families use it:')
    ) {
...
function buildJourneyTimelineHtml({ amountDue = 0, showPaymentMethods = true, ctaHref = 'https://summer.newushu.com/register' }) {
  const isPaymentJourney = showPaymentMethods && Number(amountDue || 0) > 0
  const steps = isPaymentJourney
    ? [
        { step: '1', title: 'Review summary', note: 'Double-check your selected weeks and family details.' },
        { step: '2', title: 'Send payment', note: 'Use the payment options below to confirm your spot.' },
        { step: '3', title: 'Get confirmed', note: 'Your schedule is locked in once payment is received.' },
      ]
    : [
        { step: '1', title: 'Review your plan', note: 'Check the current weeks already reserved or enrolled.' },
        { step: '2', title: 'Add more weeks', note: 'Use the registration page to extend the summer schedule.' },
        { step: '3', title: 'Build momentum', note: 'More weeks create stronger skill growth and consistency.' },
      ]

  return `
    <div style="margin:18px 0 0;padding:18px 18px 16px;border:1px solid #fde68a;border-radius:20px;background:linear-gradient(180deg,#fffdf4 0%,#fff8df 100%);box-shadow:0 10px 24px rgba(234,88,12,0.05);">
      <p style="margin:0 0 12px;font-size:13px;font-weight:900;letter-spacing:0.05em;text-transform:uppercase;color:#b45309;">What Happens Next</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;">
        ${steps
          .map(
            (item) => `
      ...
      <p style="margin:14px 0 0;font-size:13px;font-weight:700;color:#7c2d12;">Registration link: <a href="${escapeHtml(ctaHref)}" style="color:#b45309;text-decoration:underline;">${escapeHtml(ctaHref)}</a></p>
    </div>
  `
}

function buildProgressDiagramHtml({ amountDue = 0, showPaymentMethods = true }) {
  const cards = showPaymentMethods && Number(amountDue || 0) > 0
    ? [
        { label: 'Reservation', value: 'Held', note: 'Your family selected weeks and submitted registration.' },
        { label: 'Payment', value: 'Needed', note: 'Payment confirms the held schedule before release.' },
        { label: 'Summer Spot', value: 'Secure', note: 'Once paid, your weeks are fully locked in.' },
      ]
    : [
        { label: 'Enrollment', value: 'Confirmed', note: 'Your current weeks are already secured.' },
        { label: 'More Weeks', value: 'Optional', note: 'You can still add more full weeks while space is open.' },
        { label: 'Progress', value: 'Build More', note: 'Longer summer plans usually create better results.' },
      ]

  return `
    <div style="margin:16px 0 0;padding:16px;border:1px solid #bfdbfe;border-radius:18px;background:linear-gradient(180deg,#f8fbff 0%,#eef6ff 100%);">
      <p style="margin:0 0 12px;font-size:13px;font-weight:900;letter-spacing:0.05em;text-transform:uppercase;color:#1d4ed8;">Journey Snapshot</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;">
        ${cards
          .map(
            (item) => `
              <div style="padding:14px 12px;border-radius:16px;border:1px solid #bfdbfe;background:#ffffff;text-align:center;box-shadow:0 6px 18px rgba(29,78,216,0.06);">
                <p style="margin:0;font-size:11px;font-weight:900;letter-spacing:0.04em;text-transform:uppercase;color:#1d4ed8;">${escapeHtml(item.label)}</p>
                <p style="margin:8px 0 6px;font-size:24px;font-weight:900;color:#0f172a;">${escapeHtml(item.value)}</p>
                <p style="margin:0;font-size:12px;line-height:1.5;color:#475569;">${escapeHtml(item.note)}</p>
              </div>
            `
          )
          .join('')}
...
function buildReservationJourneyEmailHtml({
  ...
  const trustBarHtml = buildTrustBarHtml()
  const offerHtml = buildFourthWeekOfferHtml('Offer Reminder')
  const footerHtml = buildEmailFooterHtml({ showReply: true })
  const journeyTimelineHtml = buildJourneyTimelineHtml({ amountDue, showPaymentMethods, ctaHref })
  const progressDiagramHtml = buildProgressDiagramHtml({ amountDue, showPaymentMethods })
  return `
      ...
      <div style="max-width:700px;margin:0 auto;background:#ffffff;border:1px solid #fde68a;border-radius:22px;overflow:hidden;box-shadow:0 24px 60px rgba(194,65,12,0.14);font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;">
        ...
        <div style="padding:26px 24px 20px;">
          ...
          <div style="margin:20px 0 0;padding:20px 22px;border:2px solid #0284c7;border-radius:18px;background:linear-gradient(135deg,#0284c7 0%,#2563eb 100%);text-align:center;box-shadow:0 12px 32px rgba(2,132,199,0.28);">
            ...
            <p style="margin:10px 0 0;font-size:13px;color:#ffffff;font-weight:700;">
              Register here: <a href="${escapeHtml(ctaHref)}" style="color:#ffffff;text-decoration:underline;">${escapeHtml(ctaHref)}</a>
            </p>
          </div>
          ${upsellHtml}
          ${levelUpHtml}
...
function buildReservationStepContent({ firstName, stepNumber, reservationDeadlineLabel, summaryLines, amountDue, payload }) {
  ...
  const bodyText = [...bodyLines, '', 'Family rewards:', ...getUpsellOfferLines(payload), '', ...getLevelUpRewardsLines(payload), '', 'Level Up app:', 'Families get access throughout the summer for lunch booking, progress photos/videos, and instructor notes.', '', 'Payment Methods:', PAYMENT_METHODS_TEXT, '', 'Registration Summary:', ...(summaryLines || [])].join('\n')
...
function formatWeekOf(dateStr) {
  ...
  if (!raw) return 'your upcoming week'
  const dayOnly = /^\d{4}-\d{2}-\d{2}$/.test(raw)
  const parsed = dayOnly ? new Date(`${raw}T12:00:00.000Z`) : new Date(raw)
...
function buildPaidPrepContent({ firstName, stage, payload }) {
  ...
  const weekOfLabel = formatWeekOf(firstWeek?.start) || 'your upcoming week'
  const commonClose = [
    '',
  ...
  const stageMap = {
    sevenDay: {
      heading: `Camp Week of ${weekOfLabel} — Preparation Snapshot`,
      subject: `Your Upcoming Camp Week (${weekOfLabel}): Prep Snapshot + Add-Weeks Offer`,
      preview: `${CAMP_NAME} — camp week of ${weekOfLabel}`,
      ctaLabel: 'View Schedule & Add More Weeks',
      ctaHref: 'https://summer.newushu.com/register',
      lines: [
        `Hi ${firstName},`,
        '',
        `Great news — ${camperLabel} is all set for camp week of ${weekOfLabel}. This is your preparation snapshot plus your exclusive registered-family add-weeks offer.`,
        '',
        `What to bring every day: athletic training clothes, water bottle (labeled), and a positive mindset. Camp hours are ${DAY_CAMP_HOURS_WITH_PICKUP_LABEL}. Tuesday outdoor time may call for sunscreen and a change of outdoor shoes. Water Wednesdays require an extra change of clothes. Thursday BBQ lunch is provided. Friday family showcase happens during pickup and is open to all families.`,
        '',
        'This is a great time to add additional camp weeks at your registered-family rate before spots fill for the rest of the summer.',
        ...commonClose,
      ],
    },
    fiveDay: {
      heading: `Camp Week of ${weekOfLabel} — Smooth Start Checklist`,
      subject: `Your Upcoming Camp Week (${weekOfLabel}): Logistics Checklist + Add a Week`,
      preview: `${CAMP_NAME} — camp week of ${weekOfLabel}`,
      ...
      lines: [
        `Hi ${firstName},`,
        '',
        `Camp week of ${weekOfLabel} is coming up for ${camperLabel}. Here is everything you need for a smooth first morning:`,
        '- Athletic training clothes and a labeled water bottle every day.',
        '- Tuesday outdoor block: pack sunscreen and a change of outdoor shoes if your camper likes them.',
        ...
        '- Friday: plan to attend the family performance showcase during pickup.',
        '',
    ...
    threeDay: {
      heading: `Camp Week of ${weekOfLabel} — Final Packing List`,
      subject: `Your Upcoming Camp Week (${weekOfLabel}): Final Packing + Arrival Details`,
      preview: `${CAMP_NAME} — camp week of ${weekOfLabel}`,
      ...
      lines: [
        `Hi ${firstName},`,
        '',
        `Camp week of ${weekOfLabel} is almost here for ${camperLabel}. Final quick checklist:`,
        '- Training clothes and water bottle (labeled) ready for each day.',
        '- Tuesday outdoor time: sunscreen and outdoor shoes/change of shoes ready if needed.',
        '- Wednesday extra change of clothes packed and ready.',
        '- Thursday: BBQ is provided — no lunch prep needed.',
        `- Camp runs ${DAY_CAMP_START_TIME}-${DAY_CAMP_END_TIME}, with pickup from ${DAY_CAMP_PICKUP_WINDOW}.`,
        '- Friday showcase happens during pickup — families are welcome and encouraged.',
        '',
    ...
    oneDay: {
      heading: `Camp Week of ${weekOfLabel} — See You Tomorrow!`,
      subject: `Your Camp Week (${weekOfLabel}) Starts Tomorrow: Final Arrival Reminder`,
      preview: `${CAMP_NAME} — camp week of ${weekOfLabel}`,
      ...
      lines: [
        `Hi ${firstName},`,
        '',
        `Camp week of ${weekOfLabel} starts tomorrow for ${camperLabel} — the whole team is excited and ready.`,
        '',
        'Last-minute checklist: training clothes, labeled water bottle, Tuesday sunscreen/outdoor shoes if needed, Wednesday change of clothes if applicable, and a note of any arrival logistics to keep the morning smooth.',
        '',
        `Camp hours are ${DAY_CAMP_HOURS_WITH_PICKUP_LABEL}. Friday family showcase happens during pickup, so mark your calendar.`,
        '',
...
function buildPaidEnrollmentFollowupContent({ firstName, stage, payload }) {
  ...
  const trackLabel = isBootcampTrack ? 'Competition Boot Camp' : 'General Camp'
  const rhetoricalQuestion = isBootcampTrack
    ? 'How do you help an athlete build sharper taolu technique and stronger competition confidence before fall?'
    : 'How do you give a camper a summer that is active, structured, and actually helps them grow week after week?'
  const solutionLine = isBootcampTrack
    ...
    : 'We solve that with structured full-day training, positive team culture, visible progress, and a weekly rhythm that helps campers keep building confidence.'
  const commonClose = [
    '',
    'Thank you again for signing up. We are excited to welcome your family and cannot wait to see you at summer camp.',
    '',
    'Train More, Save More: adding more full weeks can unlock larger week-based savings for your family.',
    'Level Up rewards: each full week earns 2,500 points.',
    '',
  ...
  const stageMap = {
    zeroWeek: {
      heading: `${trackLabel} Registration Confirmed`,
      subject: isBootcampTrack
        ? 'Your Competition Boot Camp Registration Is Confirmed'
        : 'Your Summer Camp Registration Is Confirmed',
      ...
      lines: [
        `Hi ${firstName},`,
        '',
        `Wonderful news. ${camperLabel} is confirmed for ${trackLabel}, and we are excited to welcome your family this summer.`,
        '',
        rhetoricalQuestion,
        solutionLine,
        '',
    ...
    twoWeek: {
      heading: `${trackLabel} Momentum Check-In`,
      subject: isBootcampTrack
        ? 'You’re Signed Up for Boot Camp. Want Even More Taolu Progress This Summer?'
        : 'You’re Signed Up for Summer Camp. Want Even More Growth This Summer?',
      preview: `${CAMP_NAME} — 2-week enrolled follow-up`,
      ctaLabel: 'Add More Weeks',
      ctaHref: 'https://summer.newushu.com/register',
      lines: [
        `Hi ${firstName},`,
        '',
        `Thank you again for signing up ${camperLabel} for ${trackLabel}. We are excited to have your family enrolled and cannot wait to see you this summer.`,
        '',
        rhetoricalQuestion,
        solutionLine,
        '',
        'Many families begin with a few weeks, then realize their camper would benefit from more consistency, more coaching, and more time with the team.',
        'That is why we encourage families to add weeks early while more schedule options are still open.',
        ...commonClose,
      ],
    },
    fourWeek: {
      heading: 'Keep the Summer Plan Growing',
      subject: isBootcampTrack
        ? 'Would More Boot Camp Weeks Help Your Athlete Progress Faster?'
        : 'Would More Camp Weeks Help Your Camper Grow Even More This Summer?',
      preview: `${CAMP_NAME} — 4-week enrolled follow-up`,
      ctaLabel: 'Review More Weeks',
      ctaHref: 'https://summer.newushu.com/register',
      lines: [
        `Hi ${firstName},`,
        '',
        `We are excited for ${camperLabel} to join us this summer, and we wanted to check in with one simple question: what usually creates the best summer results?`,
        '',
        isBootcampTrack
          ? 'More repetition, more corrections, and more time under focused coaching. That is how athletes clean up technique and gain stronger competition confidence.'
        ...
        isBootcampTrack
          ...
          : 'If your camper thrives with structure and movement, extra full weeks often make the whole summer smoother and more productive.',
        ...commonClose,
      ],
    },
    eightWeek: {
      heading: 'One More Summer Schedule Nudge',
      subject: isBootcampTrack
        ? 'One More Boot Camp Planning Check-In Before More Weeks Fill'
        : 'One More Summer Camp Planning Check-In Before More Weeks Fill',
      preview: `${CAMP_NAME} — 8-week enrolled follow-up`,
      ...
      lines: [
        ...
        '',
        isBootcampTrack
          ? 'What helps an athlete feel more ready for performance season: one or two solid weeks, or a longer runway of technical reps and focused coaching? We believe the stronger runway wins.'
...
function getReservationPreviewPayload() {
  return {
    ...
    summaryLines: [
      ...
      `Offer reminder: ${WEEK_TIER_PROMO.headline}`,
...
export function buildReservationJourneyMessage({
  ...
  if (safeStepNumber >= 6) {
    ...
    const bodyText = [...getWeekTierPromoLines(), '', ...content.lines].join('\n')
    ...
    return { subject: publicWushuCopy(content.subject), bodyText: publicWushuCopy(bodyText), html: publicWushuCopy(html) }
  }
  ...
  const bodyText = [...getWeekTierPromoLines(), '', ...content.bodyText.split('\n')].join('\n')
  const html = buildReservationJourneyEmailHtml({
    heading: content.heading,
    preview: content.preview,
...
export function buildPaidEnrollmentJourneyMessage({
  stepNumber,
  logoUrl = '',
  landingCarouselImageUrls = [],
  payload = getReservationPreviewPayload(),
}) {
  const safeStepNumber = Math.max(1, Math.min(4, Number(stepNumber || 1)))
  const stageIds = ['zeroWeek', 'twoWeek', 'fourWeek', 'eightWeek']
  const firstName = 'Calvin'
  const summaryLines = Array.isArray(payload?.summaryLines) ? payload.summaryLines : []
  const stage = { id: stageIds[safeStepNumber - 1] || 'zeroWeek' }
