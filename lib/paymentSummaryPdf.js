...
export function buildPaymentSummaryPdfBase64({
  ...
  function drawText(text, x, y, options = {}) {
    const { font = 'F1', fontSize = 11, color = [0.071, 0.102, 0.161] } = options
    push('BT')
  ...
  function startPage() {
    ...
    drawText(title, margin + 14, pageHeight - margin - 26, {
      font: 'F2',
      fontSize: 22,
  ...
  function drawWrappedLine(line, options = {}) {
    const { font = 'F1', fontSize = 11, lineHeight = 15, indent = 0, color = [0.2, 0.239, 0.314] } = options
    const wrapped = wrapText(line, maxChars - Math.round(indent / 4))
    ensureSpace(wrapped.length * lineHeight + 4)
    wrapped.forEach((part) => {
      drawText(part, margin + indent, cursorY, { font, fontSize, color })
      cursorY -= lineHeight
    })
  }

  function drawSection(section) {
    const wrappedLines = section.lines.flatMap((line) => wrapText(line, maxChars - 6))
    const titleHeight = 24
    const bodyPadding = 12
    const lineHeight = 15
    const bodyHeight = bodyPadding * 2 + wrappedLines.length * lineHeight
    const sectionHeight = titleHeight + bodyHeight
    ensureSpace(sectionHeight + 12)
    drawRect(margin, cursorY - sectionHeight, contentWidth, sectionHeight, {
      fillColor: [0.995, 0.998, 1],
    ...
    drawText(section.title, margin + 12, cursorY - 7, {
      font: 'F2',
      fontSize: 13,
      color: [0.604, 0.204, 0.024],
    })
    cursorY -= titleHeight + bodyPadding + 3
    section.lines.forEach((line) => {
      drawWrappedLine(line, {
        fontSize: 11,
        lineHeight,
        indent: 12,
        color: [0.2, 0.239, 0.314],
      })
    })
    cursorY -= bodyPadding
  }
