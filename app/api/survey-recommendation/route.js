function buildFallbackRecommendation(surveyData, recommendationLines) {
  const ages = Array.isArray(surveyData?.camperAges)
    ? surveyData.camperAges.map((age) => Number(age || 0)).filter((age) => age > 0)
    : []
  const hasCompetitionGoal = Array.isArray(surveyData?.goals) && surveyData.goals.includes('competition')
  const hasSocialGoal =
    Array.isArray(surveyData?.goals) && surveyData.goals.includes('social/teamwork/friends')
  ...
  const leadLine = allUnder6
    ? 'General Camp is the strongest starting point for your family right now.'
    : hasCompetitionGoal
      ? 'A smart path is Competition Team Boot Camp, with General Camp weeks mixed in as needed.'
      : 'General Camp is a great fit, with the option to add Competition Team Boot Camp as skills progress.'
  ...
  const logisticsLine = hasLunchNeed
    ? 'For convenience, you can select lunch by day during registration and build a schedule that fits your week.'
    : 'You can register by week and still keep flexibility with day-by-day scheduling choices.'

  const nextStep = recommendationLines?.[0]
    ? `Next step: ${recommendationLines[0].replace(/^Best fit:\s*/i, '')}`
    : 'Next step: open registration and choose weeks to get your preferred spots.'
  const marketLine =
...
export async function POST(request) {
  try {
    ...
    const text = isWeakRecommendation(textRaw)
      ? buildFallbackRecommendation(surveyData, recommendationLines)
      : textRaw
    ...
    return Response.json({ configured: true, error: error.message || 'Request failed.' }, { status: 500 })
  }
}
